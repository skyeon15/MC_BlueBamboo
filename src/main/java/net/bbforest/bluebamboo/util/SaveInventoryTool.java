...
public class SaveInventoryTool {
    ...
    public static void saveInventoryData() {
        ...
        if (same) Tool.Log.MSG.send("중복을 제외한 인벤토리 저장 완료! " + date);
        else Tool.Log.MSG.send("인벤토리 저장 완료! " + date);
        updateList();
    }
    ...
    public static void autoSaveInventoryData() {
        ...
        Bukkit.getScheduler().runTaskTimer(EKE.getPlugin(), () -> {
            ...
            if (Bukkit.getOnlinePlayers().isEmpty()) {
                //Tool.Log.MSG.send("온라인인 사용자가 없어서 인벤토리를 저장하지 않았어요.");
                return;
            }
            if (!config.getBoolean("inventory-snapshot.enabled")) {
                return;
            }
            Tool.Log.MSG.send("인벤토리 자동 저장 중...");
            saveInventoryData();
